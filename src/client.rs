...
pub async fn make_request(
    ...
    console_log!(
        "🚀 HTTP request starting: {}:{} (Protocol: {:?})",
        host,
        port,
        protocol
    );
    ...
    match scheme {
        "https" => {
            ...
            let (tls_stream, negotiated_protocol) =
                create_tls_connection_with_alpn(&host, socket, &protocol).await?;

            console_log!(
                "🔒 TLS handshake completed, negotiated protocol: {:?}",
                negotiated_protocol
            );
            ...
            match negotiated_protocol {
                Some(protocol) => {
                    ...
                    match protocol.as_str() {
                        "h2" => {
                            ...
                            make_http2_response(
                                &host, path, method, &headers, body, tls_stream, scheme,
                            )
                            .await
                        }
                        "http/1.1" => {
                        ...
                        _ => {
                            console_log!(
                                "⚠️ Unknown protocol: {:?}, falling back to HTTP/1.1",
                                protocol
                            );
                            make_http1_response(
                                &host, path, method, &headers, body, tls_stream, scheme,
                ...
                None => {
                    ...
                    make_http1_response(&host, path, method, &headers, body, tls_stream, scheme)
                        .await
                }
            }
        }
